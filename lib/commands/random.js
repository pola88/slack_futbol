  ...
  run() {
    pgConnection.query("SELECT * FROM players ORDER BY created_at ASC LIMIT 12;", (error, result) => {
      ...
      if(_.isEmpty(players)) {
        this.slack.replyWithTyping(this.payload, this._buildPayload("No hay jugadores, son todos putos!!"));
        return;
      } else if(players.length < 8) {
        this.slack.replyWithTyping(this.payload, this._buildPayload("Con la cantidad que son, jueguen al Fifa en la play, no molesten."));
        return;
      }
        ...
        pgConnection.query(`UPDATE players SET team = 'A' where id IN (${userIdsForQuery});`, () => {
         ...
         pgConnection.query(`UPDATE players SET team = 'B' where id IN (${userIdsForQuery})`, async () => {
           ...
           if(this.payload.telegram) {
             ...
             this.slack.replyWithTyping(this.payload, this._buildPayload(`*Sol en la cara* (#000000) ${teamANames.join(", ")}\n*Sol en la nuca* (#fafafa) ${teamBNames.join(", ")}`));
           }
         });
        });
      });
