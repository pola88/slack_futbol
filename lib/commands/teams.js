      ...
      Slack.getUsers(userIds)
         .then( userInfo => {
           ...
           if(this.payload.telegram) {
             deferred.resolve(this._buildPayload(`Sol en la cara (Negra): ${teamA.join(", ")}\nSol en la nuca(Blanca): ${teamB.join(", ")}` ));
           } else {
            deferred.resolve(this._buildPayload(`Sol en la cara (#000000): ${teamA.join(", ")}\nSol en la nuca(#fafafa): ${teamB.join(", ")}` ));
           }
